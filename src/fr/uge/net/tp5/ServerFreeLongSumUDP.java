...
import static fr.uge.net.tp5.LongSumPacket.*;
...
public class ServerFreeLongSumUDP {
     ...
     * Suppose buffer in read mode
     * */
    private void processClean(InetSocketAddress sender) throws IOException {
        if (buffer.limit() - buffer.position() != Long.SIZE / 8) {
            return;
        }
        var sessionId = buffer.getLong();
        ...
        try {
            while (!Thread.interrupted()) {
                ...
                if (buffer.position() >= Byte.SIZE / 8) {
                    ...
                    var b = buffer.get();
                    switch (b) {
                        case 1 -> processOp(sender);
                        case 4 -> processClean(sender);
                    }
                }
            }
        } finally {
